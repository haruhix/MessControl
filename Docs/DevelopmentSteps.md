...
**26.09.2026 — подвижный язык:** совпадающая деформация и коллизия, самостоятельное движение в покое, красная волна боли от язв и толчок игроков/еды. Проверка через F3, настройки в `DA_Tongue`. [Назначение объектов и инструкция](Tongue.md), [видео](../Artifacts/TonguePain.mp4).
...
| 7. Жидкость — прототип кофе сделан; зевание и движение впереди | Струя, наполнение, слив в глотку, плавучесть ragdoll и зацеп | [Кофе](CoffeeWater.md) |
...
## Кофейная поверхность и плавание — 25.09.2026

**Зачем:** волна должна быть видна там, где она физически поддерживает персонажа. Кофе получил сетку с волнами, прозрачность по глубине, движущиеся блики, пену и круги возле игроков. Физическая высота волн использует те же параметры, что и материал; ragdoll сохраняет течение, управление и зацеп.

`MI_CoffeeSurface` отвечает за внешний вид, `DA_CoffeeWater` — за волны и плавучесть, `DA_Day01` — за течение и силу подгребания. В обновлении ниже длительность кофе также перенесена в профиль воды. [Настройки, управление и запись](CoffeeWater.md).

## Струя кофе и слив в глотку — 26.09.2026

**Зачем:** событие должно сначала расталкивать игроков от падающей струи, а затем менять направление и тянуть к глотке. Вместо прежних четырёх подъёмов теперь по умолчанию один цикл: 4 секунды наполнения, 2 секунды слива. Повторы и времена находятся в `DA_CoffeeWater`.

Добавлены формы струи, короны брызг, капель и полосы слива, отдельные материалы для них. `MCCoffeeVisuals.cpp` отвечает за изображение; `MCCoffeeFlood` — за игровые силы и первый удар, `MCCoffeeProfile` — за общие параметры. Это позволяет художнику заменить формы и настроить цвет, не переписывая механику. Вода остаётся управляемым игровым эффектом без fluid solver. Карта и модели коллеги сохранены. [Полный разбор и проверка через F3](CoffeeWater.md).

## Адаптация новой арены — 26.09.2026

Из двух цельных рядов коллеги выделены отдельные зубы с сохранением формы и UV. Восемь передних получили уход, статусы и расходование на возрождение; два дальних остались декорациями. Старый плоский пол заменён коллизией по форме нового языка. Пятна, язвы, выходы еды/щёток и отметки кофе связаны с этой геометрией. [Что создано, зачем и где редактировать](ArtistArena.md).

## Движение языка — 26.09.2026

**Зачем:** язык должен быть игровой поверхностью, которая реагирует на боль и
может сбросить игроков. Наступание на язву запускает красную волну; редкий
сильный рывок поджимает и поднимает переднюю часть языка, бросает игроков и
еду. Видимая сетка и коллизия меняются вместе. `DA_Tongue` задаёт паузы,
высоту и силу; две кнопки F3 позволяют проверить реакции отдельно.
[Устройство, настройки и видео](Tongue.md).

## Глаза и профили событий языка — 26.09.2026

**Зачем:** персонажи должны замечать друг друга и происходящее, а события
должны вызывать разные движения языка через один интерфейс. `MCGazeComponent`
выбирает цель взгляда и анимирует существующие глаза/веки; `DA_Gaze` и F1
задают выразительность. `MCTongueMotionProfile` хранит форму, силу и времена
движения. `PlayMotion` принимает профиль, мировую точку, направление и масштаб;
язва и автоматический рывок используют тот же путь. В F3 доступны локальный
подъём и направленная волна. [Глаза](Gaze.md), [события языка](Tongue.md).

## Вес на поверхности языка — 26.09.2026

**Зачем:** игроки и переносимая еда должны ощущаться тяжёлыми. Под контактирующими
объектами появляются небольшие вмятины; после движения язык выпрямляется.
Масса, площадь контакта и приземление влияют на результат. После уточнения
используется WPO: вмятины визуальные, физический пол от веса не меняется.
Сильные движения событий отдельно меняют геометрию и коллизию. Настройки —
`DA_Tongue.Pressure`, сравнение лёгкого и тяжёлого предметов — F3.
[Как проверить, где менять и границы приближения](TonguePressure.md).

## Контакт рук и визуальный вес — 26.09.2026

**Зачем:** убрать тягу на расстоянии и связать позу с настоящей поверхностью
еды. `MCGripComponent` хранит контакты ладоней на предмете, подводит руки,
выбирает боковую / переднюю / заднюю позу, ограничивает длину цепочки и
передаёт усилие только после завершения подвода. Занятые руки временно
управляются IK; отпускание и ragdoll возвращают физику. Параметры — DA_Grip,
тестовый предмет — F3. [Подробности и границы](Grip.md).

По уточнению веса вмятины переведены в WPO. Материал читает глубину из UV1,
а базовые позиции вершин и физический пол от веса не изменяются. Сильные
движения языка продолжают менять геометрию вместе с коллизией.
