...
**Зачем:** будущему возрождению нужен конкретный зуб, который можно потерять или потратить. Теперь на арене появляются восемь небольших игровых зубов с номерами 01–08. Большие зубы старого блокинга остаются декорациями. Карта и добавленная вручную модель `teeth_low` не перезаписываются.
...
1. Открой `L_Mouth`, нажми Play. Вдоль краёв арены появятся небольшие зубы с номерами, HUD покажет `ARENA TEETH 8 / 8`.
2. Нажми F1, прокрути вниз до `ARENA TEETH / HOST`, нажми `PREVIEW COFFEE ON ARENA`. Закрой F1: на зубах виден налёт.
3. Подойди к одному зубу, повернись к нему и нажми ПКМ четыре раза с паузами. После третьего удара он шатается, после четвёртого выпадает; остаётся 7/8.
...
| [MCGameMode.cpp](../Source/MessControl/Private/MCGameMode.cpp) | Создаёт зубы при старте забега из `DA_RunRules` и `DA_ArenaTooth` |
