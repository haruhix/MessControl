...
if ($Mode -eq 'Unit') {
    ...
    if ($taskReport.failed -ne 0 -or ($taskReport.succeeded + $taskReport.succeededWithWarnings) -lt 27) { throw 'Not all gameplay and physics tests passed.' }
    ...
    try {
        for ($taskIndex=0; $taskIndex -lt 4; $taskIndex++) {
            ...
            if($CapturePressure -and $Mode -eq 'TonguePressureNetwork' -and $taskIndex -eq 0) {
                ...
                $taskArguments+=@('-MCTonguePressureCapture','-RenderOffscreen','-windowed','-ForceRes','-ResX=1280','-ResY=720','-NoScreenMessages','-ExecCmds="t.MaxFPS 60,t.IdleWhenNotForeground 0"')
            }
