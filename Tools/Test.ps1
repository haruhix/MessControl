param([string]$EngineRoot=$env:UE_ROOT,[ValidateSet('Unit','Network','CoreNetwork','DayOneNetwork','DevPanelNetwork','CoffeeNetwork','TongueNetwork','ArenaNetwork','Visual','Ragdoll','RagdollVisual','Limbs')][string]$Mode='Unit',[ValidateRange(0,250)][int]$PacketLagMs=0,[ValidateRange(0,10)][int]$PacketLoss=0,[ValidateSet(30,60,120)][int]$FrameRate=60,[switch]$CaptureCore,[switch]$CaptureDayOne,[switch]$CaptureDevPanel,[switch]$CaptureCoffee,[switch]$CaptureTongue)
...
if ($Mode -eq 'Unit') {
    ...
    if ($taskReport.failed -ne 0 -or ($taskReport.succeeded + $taskReport.succeededWithWarnings) -lt 22) { throw 'Not all gameplay and physics tests passed.' }
    ...
    try {
        for ($taskIndex=0; $taskIndex -lt 4; $taskIndex++) {
            ...
            if($Mode -eq 'TongueNetwork') { $taskArguments+='-MCTongueTest' }
            if($CaptureTongue -and $Mode -eq 'TongueNetwork' -and $taskIndex -eq 0) {
